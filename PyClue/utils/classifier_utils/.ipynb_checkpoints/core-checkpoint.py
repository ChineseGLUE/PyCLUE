...
class UserConfigs(TaskConfigs):
    ...
    def __init__(self, configs):
        self.label_position = configs.get("label_position")
        self.text_a_position = configs.get("text_a_position")
        self.text_b_position = configs.get("text_b_position")
...
class ClassificationProcessor(DataProcessor):
    ...
    def __init__(self, labels, label_position, text_a_position, text_b_position=None, ignore_header=True, min_seq_length=None, file_type="tsv", delimiter="\t"):
        self.language = "zh"
        self.labels = labels
        self.label_position = label_position
        self.text_a_position = text_a_position
        self.text_b_position = text_b_position
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_file(os.path.join(data_dir, "train."+self.file_type), delimiter=self.delimiter), "train"
        )
    
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_file(os.path.join(data_dir, "dev."+self.file_type), delimiter=self.delimiter), "dev"
        )
    
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_file(os.path.join(data_dir, "test."+self.file_type), delimiter=self.delimiter), "test"
        )
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i, line in enumerate(lines):
            guid = "%s-%s" %(set_type, i)
            try:
                label = tokenization.convert_to_unicode(line[self.label_position])
                text_a = tokenization.convert_to_unicode(line[self.text_a_position])
                text_b = None if not self.text_b_position else tokenization.convert_to_unicode(line[self.text_b_position])
                examples.append(
